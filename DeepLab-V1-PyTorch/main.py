...
batch_size = 1 # 30 for "step", 10 for 'poly'
lr = 1e-3
weight_decay = 5e-4
...
num_save_iters = 1000
num_print_iters = 100
init_model_path = './exp/model_last_20000_clothes.pth'
log_path = './exp/log.txt'
model_path_save = './exp/model_last_'
root_dir_path = '/root/segmentation/dataset'
crop_pred_dir_path = './exp/labels/clothes/crop/'
original_pred_dir_path = './exp/labels/human/original/'
...
def train():
    ...
    train_loader = torch.utils.data.DataLoader(
        VOCDataset(split='train_aug', crop_size=321, is_scale=False, is_flip=True, is_train=True),
    ...
    for epoch in range(1, 100):
        for iter_id, batch in enumerate(train_loader):
            ...
            if iters % num_save_iters == 0:
                torch.save(model.state_dict(), model_path_save + str(iters) + '_clothes.pth')
...
def test(model_path_test, use_crf):
    ...
    val_loader = torch.utils.data.DataLoader(
        VOCDataset(split='val', crop_size=crop_size, label_dir_path='clothes_labels', is_scale=False, is_flip=False, is_train=False),
        batch_size=batch_size,
        shuffle=False,
        num_workers=4,
        drop_last=False
    ...
    img_dir_path = root_dir_path + '/images/'
    # class palette for test
    palette = []
    for i in range(256):
        palette.extend((i,i,i))
    palette[:3*21] = np.array([[0, 0, 0],
                            [128, 128, 128]],dtype='uint8').flatten()
    ...
    for iter_id, batch in enumerate(val_loader):
        image_ids, images, labels, (start_h, start_w) = batch
        images = images.to(device)
        labels = losses.resize_labels(labels, size=(crop_size, crop_size)).to(device)
        logits = model(images)
        probs = nn.functional.softmax(logits, dim=1) # shape = [batcdh_size, C, H, W]
        outputs = torch.argmax(probs, dim=1) # shape = [batch_size, H, W]
        labels=labels/15
        loss_seg = CEL(logits, labels.long())
        ...
        for i in range(len(image_ids)):
            if is_post_process:
                raw_image = cv2.imread(img_dir_path + image_ids[i][:-4] + '.jpg', cv2.IMREAD_COLOR) # shape = [H, W, 3]
                ...
                raw_image = raw_image[start_h:start_h+crop_size, start_w:start_w+crop_size, :]
            ...
            img_label.save(crop_pred_dir_path + image_ids[i][:-3] + 'png')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path_test', default='./exp/model_last_20000_clothes.pth', help='test model path')
